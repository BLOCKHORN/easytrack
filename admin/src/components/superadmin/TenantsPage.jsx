...
/* ====== Etiquetas de planes ======
   * Nuevo set: Mensual / Trimestral / Anual
   * Mantenemos alias antiguos por compatibilidad (quarterly ya soporta "trimestral") */
const PLAN_LABEL = {
  monthly: "Mensual",
  mensual: "Mensual",
  monthlyy: "Mensual",
  ...
  annual: "Anual",
  anual: "Anual",
  // alias legacy (redirigimos a Anual para no romper)
...
export default function TenantsPage() {
  ...
  // pestañas internas: "data" | "logs"
  const [tab, setTab] = useState("data"); // por UX, dejamos Datos seleccionado cuando pulsen la pestaña

  async function openTenant(t) {
    const { tenant, subscription } = await getTenant(t.id);
    setSelected({ ...tenant, subscription });
    setTab("data"); // Resumen siempre visible arriba; la pestaña controla el panel inferior
  ...
  async function refreshUsers(text = "") {
    try {
      const { users: found } = await searchUsers(text);
      setUsers(found || []);
    } catch {
      setUsers([]);
    }
  ...
  async function onReset(email) {
    try {
      ...
      alert("No se pudo generar el reset.");
    }
  }
  ...
  function openEdit(row) {
    ...
    // campos editables controlados (whitelist básica en UI)
  ...
  async function applyEdit() {
    if (!editTarget?.field) {
      alert("Selecciona un campo editable.");
      return;
    }
    try {
      ...
      alert("Campo no permitido por backend o error al actualizar.");
    }
  }
  ...
  return (
    <div className="tenants tenants--clean">
      {/* ===== Sidebar ===== */}
      <aside className="tz-side">
        <div className="tz-side__head">
          <h3>
            <Icon name="building" /> Tenants
          </h3>
        ...
        <div className="tz-side__list">
          {loading ? (
            <div className="muted p12">Cargando…</div>
          ) : pageRows.length === 0 ? (
                ...
                >
                  ...
                  <div className="tz-txts">
                    ...
                    <div className="meta">
                      ...
                      <Chip
                        tone={
                          s === "active" ? "ok" : s === "trialing" ? "info" : s === "past_due" ? "warn" : "muted"
                        }
                      >
                        {s}
                      </Chip>
      ...
      <main className="tz-main">
        {!selected ? (
          <Empty title="Selecciona un tenant" sub="Usa la búsqueda y los filtros de la izquierda." />
        ) : (
          <>
            {/* HEADER */}
            ...
            {/* ===== Pestañas (encima del wrap) ===== */}
            <div className="tz-tabs">
              <button className={`tz-tab ${tab === "data" ? "is-active" : ""}`} onClick={() => setTab("data")}>
                <Icon name="db" /> Datos
              </button>
              <button className={`tz-tab ${tab === "logs" ? "is-active" : ""}`} onClick={() => setTab("logs")}>
                <Icon name="list" /> Auditoría
              </button>
            </div>

            {/* ===== Resumen SIEMPRE visible (no pestaña) ===== */}
            <section className="tz-summary">
              <div className="tz-cards-3 tz-same">
                {/* Estado */}
                <section className="tz-card">
                  <header className="tz-card__head">
                    <Icon name="id" />
                    <h3>Estado</h3>
                  </header>
                  <div className="tz-kv">
                    ...
                    <KV
                      label="Estado"
                      value={
                        <span className={`pill pill--${selected.subscription?.status || "muted"}`}>
                          {selected.subscription?.status || "—"}
                        </span>
                      }
                    />
                    <KV
                      ...
                      value={
                        selected.subscription?.current_period_start
                          ? new Date(selected.subscription.current_period_start).toLocaleString()
                          : "—"
                      }
                    />
                    <KV
                      ...
                      value={
                        selected.subscription?.current_period_end
                          ? new Date(selected.subscription.current_period_end).toLocaleString()
                          : "—"
                      }
                    />
                    <KV
                      label="Fin trial"
                      value={
                        selected.subscription?.trial_ends_at
                          ? new Date(selected.subscription.trial_ends_at).toLocaleString()
                          : "—"
                      }
                    />
                  </div>
                ...
                <section className="tz-card">
                  ...
                  <div className="tz-row wrap gap8">
                    <button className="tz-btn" onClick={() => onExtend(30)}>
                      +30 días
                    </button>
                    <button className="tz-btn" onClick={() => onExtend(90)}>
                      +90 días
                    </button>
                    <button className="tz-btn" onClick={() => onExtend(365)}>
                      +365 días
                    </button>
                    <button
                      className="tz-btn tz-btn--ghost"
                      onClick={() => {
                        ...
                        alert(
                          `Periodo actual:\nInicio: ${
                            s?.current_period_start ? new Date(s.current_period_start).toLocaleString() : "—"
                          }\nFin: ${
                            s?.current_period_end ? new Date(s.current_period_end).toLocaleString() : "—"
                          }\nTrial: ${
                            s?.trial_ends_at ? new Date(s.trial_ends_at).toLocaleString() : "—"
                          }`
                        );
                      }}
                    >
                  ...
                  <div className="tz-row wrap gap8">
                    <button className="tz-btn tz-btn--ghost" onClick={() => onSetPlan("monthly")}>
                      Mensual
                    </button>
                    <button className="tz-btn tz-btn--ghost" onClick={() => onSetPlan("quarterly")}>
                      Trimestral
                    </button>
                    <button className="tz-btn tz-btn--ghost" onClick={() => onSetPlan("yearly")}>
                      Anual
                    </button>
                  </div>
                  <div className="tz-row">
                    {selected.subscription?.cancel_at_period_end ? (
                      <button className="tz-btn" onClick={onResume}>
                        Reanudar
                      </button>
                    ) : (
                      <>
                        <button className="tz-btn tz-btn--ghost" onClick={() => onCancel(true)}>
                          Cancelar al final
                        </button>
                        <button
                          className="tz-btn"
                          onClick={() => {
                            if (!confirm("¿Cancelar inmediatamente?")) return;
                            onCancel(false);
                          }}
                        >
                          Cancelar ahora
                ...
                <section className="tz-card">
                  ...
                  {!assumeSession ? (
                    <button className="tz-btn tz-btn--primary" onClick={() => onAssume("Soporte al cliente")}>
                      <Icon name="login" /> Acceder al panel del tenant
                    </button>
                  ) : (
                    <button className="tz-btn tz-btn--primary" onClick={onEndAssume}>
                      <Icon name="logout" /> Finalizar sesión de soporte
                    </button>
                  )}
                      ...
                      {users.map((u) => (
                        <div key={u.id} className="tz-tr tz-grid-3">
                          ...
                          <div className="actions">
                            <button className="tz-btn tz-btn--ghost" onClick={() => onReset(u.email)}>
                              Reset pass
                            </button>
                            <button className="tz-btn" onClick={() => onImpersonate(u.email)}>
                              Magic link
                            </button>
                          </div>
                        </div>
            ...
            {/* ===== Paneles de pestañas (scroll interno, sin desbordes) ===== */}
            <section className="tz-tabpanels">
              {tab === "data" && (
                <section className="tz-card">
                  <header className="tz-card__head">
                    <Icon name="db" />
                    <h3>Datos del tenant</h3>
                  </header>
                  ...
                  <div className="tz-toolbar">
                    ...
                    <div className="tz-input-icon">
                      <Icon name="search" />
                      <input placeholder="Buscar en tabla…" value={tableQ} onChange={onTableSearch} />
                    </div>

                    <div className="spacer" />
                    <button className="tz-btn tz-btn--ghost" onClick={refreshTable}>
                      <Icon name="refresh" />
                      Refrescar
                    </button>
                  </div>
                  ...
                  <p className="muted tiny">
                    {tableLoading ? "Cargando…" : `Mostrando datos para tenant_id = ${selected.id}`}
                  </p>
                      ...
                      <>
                        ...
                        {tableRows.map((r, idx) => (
                          <div key={r.id ?? r.uuid ?? idx} className="tz-tr tz-grid-auto">
                            {visibleCols.map((k) => (
                              <div key={k} className="ell">
                                {String(r?.[k] ?? "")}
                              </div>
                            ))}
                            <div className="actions">
                              <button className="tz-btn tz-btn--ghost" onClick={() => openEdit(r)}>
                                Editar
                              </button>
                            </div>
                          </div>
                        ...
                        <div className="tz-pager">
                          ...
                          <button
                            className="tz-btn tz-btn--ghost"
                            disabled={tablePage * 20 >= tableCount}
                            onClick={() => {
                              setTablePage((p) => p + 1);
                              setTimeout(refreshTable, 0);
                            }}
                          ...
                          <div key={a.id} className="tz-tl__item">
                            ...
                            <div className="tz-tl__card">
                              ...
                              <div className="tz-tl__body">
                                ...
                                <span className="mono">{a.target_id || "—"}</span> ·{" "}
                                <span className="muted tiny">Actor:</span>{" "}
                                <span className="mono">{a.actor_user_id ? a.actor_user_id.slice(0, 8) : "—"}</span>
                              </div>
                            </div>
                          </div>
                        ))}
      ...
      {/* ===== MODAL EDITAR FILA (Datos) ===== */}
      {editOpen && (
        <div className="tz-modal" onClick={() => setEditOpen(false)}>
          <div className="tz-modal__card" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="tz-modal__footer">
              <button className="tz-btn tz-btn--ghost" onClick={() => setEditOpen(false)}>
                Cancelar
              </button>
              <button className="tz-btn" onClick={applyEdit}>
                Guardar
              </button>
            </div>
          </div>
