...
const IconLightbulb = () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M15 14c.2-1 .7-1.7 1.5-2.5 1-.9 1.5-2.2 1.5-3.5A6 6 0 0 0 6 8c0 1 .2 2.2 1.5 3.5.7.9 1.2 1.5 1.5 2.5"/><path d="M9 18h6"/><path d="M10 22h4"/></svg>;
const IconCheckCircle = () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>;
...
export default function AnadirPaquete({ modoRapido = false, paquetes: propsPaquetes, actualizarPaquetes }) {
  ...
  const guardar = useCallback(async (e) => {
    ...
    try {
      ...
      await crearPaqueteBackend(payload, token);
  ...
  }, [loading, tenant, cliente, compania, slotSel, flyFromInputToSlot, actualizarPaquetes]);
  ...
  }, [multiSaving, multiNames, multiCount, batchSameCompany, batchCompany, multiCompanies, compania, tenant, slotSel, getMostEmptySlot, actualizarPaquetes]);
  ...
  return (
    <div className={`bg-white relative ${modoRapido ? '' : 'p-6 md:p-8 rounded-[2rem] border border-zinc-200/80 shadow-sm max-w-4xl mx-auto'}`}>
      ...
      {!modoRapido && (
        <div className="mb-8 flex items-center gap-4 border-b border-zinc-100 pb-6">
          <div className="w-12 h-12 bg-zinc-950 rounded-2xl flex items-center justify-center text-white shadow-md"><IconBox /></div>
          <div>
            <h2 className="text-2xl font-black text-zinc-950 tracking-tight">Añadir Entrada</h2>
            <p className="text-zinc-500 font-medium text-sm">Registra paquetes y el sistema sugiere la mejor ubicación.</p>
          </div>
        </div>
      )}

      {/* TABS */}
      <div className="flex bg-zinc-100/80 p-1 rounded-xl border border-zinc-200/60 mb-6 w-full max-w-sm">
        <button type="button" onClick={() => setActiveTab('single')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all ${activeTab === 'single' ? 'bg-white text-zinc-950 shadow-sm' : 'text-zinc-500 hover:text-zinc-700'}`}>Individual</button>
        <button type="button" onClick={() => setActiveTab('multi')} className={`flex-1 py-2 text-sm font-bold rounded-lg transition-all flex items-center justify-center gap-2 ${activeTab === 'multi' ? 'bg-white text-zinc-950 shadow-sm' : 'text-zinc-500 hover:text-zinc-700'}`}><IconLayers /> Múltiple</button>
      </div>

      <form onSubmit={activeTab === 'single' ? guardar : (e) => { e.preventDefault(); guardarMultiple(); }} className="space-y-6">
        
        {/* ZONA DATOS */}
        {activeTab === 'single' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            <div className="space-y-2 relative">
              <label className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Nombre del Cliente</label>
              <input 
                ref={inputClienteRef} type="text" value={cliente} 
                onChange={e => { setLeadingName(e.target.value); setSeleccionManual(false); }} 
                className={`w-full px-4 py-3 bg-zinc-50 border rounded-xl outline-none font-bold text-zinc-900 transition-all focus:ring-2 focus:ring-brand-500 ${sugCliente ? 'border-brand-300 shadow-[0_0_15px_rgba(20,184,166,0.15)]' : 'border-zinc-200'}`} 
              ...
              <AnimatePresence>
                {sugCliente && toUpperVis(leadingName) !== toUpperVis(sugCliente.name) && (
                  <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="absolute z-10 w-full mt-1">
                    <button type="button" onClick={aceptarSugerenciaCliente} className="w-full text-left px-4 py-3 bg-brand-50 border border-brand-200 rounded-xl shadow-lg flex items-center gap-3 hover:bg-brand-100 transition-colors group">
              ...
              {matchInfo?.label && (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2 mt-2 text-xs font-bold text-amber-600 bg-amber-50 px-3 py-2 rounded-lg border border-amber-200">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Compañía Logística</label>
              <select value={compania} onChange={e => setCompania(e.target.value)} className="w-full px-4 py-3 bg-zinc-50 border border-zinc-200 rounded-xl focus:ring-2 focus:ring-brand-500 outline-none font-bold text-zinc-900 transition-all cursor-pointer">
          ...
          <div className="bg-zinc-50/50 p-5 rounded-2xl border border-zinc-100 space-y-4">
             <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
               <div className="flex items-center gap-3">
                 ...
                 <select value={multiCount} onChange={(e)=> applyNewMultiCount(e.target.value)} className="px-3 py-2 bg-white border border-zinc-200 rounded-lg text-sm font-bold outline-none cursor-pointer focus:ring-2 focus:ring-brand-500">
               ...
               <label className="flex items-center gap-2 text-sm font-bold text-zinc-700 cursor-pointer">
                 <input type="checkbox" checked={batchSameCompany} onChange={e => setBatchSameCompany(e.target.checked)} className="rounded text-brand-600 focus:ring-brand-500" />
                 Misma compañía
               </label>
             </div>
             
             {batchSameCompany && (
                <select value={batchCompany} onChange={e => setBatchCompany(e.target.value)} className="w-full px-4 py-2.5 bg-white border border-zinc-200 rounded-xl font-bold text-zinc-900 outline-none focus:ring-2 focus:ring-brand-500">
                  {companias.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
             )}
             ...
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
               {multiNames.map((n, i) => (
                  <div key={i} className="flex gap-2">
                    <input type="text" value={n} onChange={e => { const v = e.target.value; setMultiNames(p => { const np=[...p]; np[i]=toUpperVis(v); return np;}); if(i===0){setLeadingName(v); setSeleccionManual(false);} }} className="w-full px-4 py-2.5 bg-white border border-zinc-200 rounded-xl focus:ring-2 focus:ring-brand-500 outline-none font-bold text-zinc-900 text-sm" placeholder={`Cliente #${i+1}...`} />
                    {!batchSameCompany && (
                      <select value={multiCompanies[i]} onChange={e => { const v=e.target.value; setMultiCompanies(p=>{ const np=[...p]; np[i]=v; return np;})}} className="w-1/3 px-2 py-2.5 bg-zinc-50 border border-zinc-200 rounded-xl text-xs font-bold outline-none">
        ...
        {/* ========================================== */}
        {/* BLOQUE CENTRAL DE DECISIÓN Y GUARDADO      */}
        {/* ========================================== */}
        <div className="flex flex-col items-center justify-center gap-6 py-6 my-6 border-y border-zinc-100">
          <div className="flex flex-row items-center justify-center gap-4 sm:gap-8 w-full">
            ...
            {/* Tarjeta Sugerencia */}
            <div className={`flex flex-col items-center justify-center w-40 sm:w-48 p-4 rounded-3xl border-2 transition-all ${suggestionPulse ? 'bg-amber-50 border-amber-300 shadow-[0_0_20px_rgba(251,191,36,0.3)]' : 'bg-zinc-50 border-zinc-200'}`}>
              <span className={`flex items-center gap-2 text-xs font-bold uppercase tracking-widest mb-3 ${suggestionPulse ? 'text-amber-600' : 'text-zinc-400'}`}>
                <IconLightbulb /> Sugerencia
              </span>
              <div className="bg-white px-6 py-2 rounded-xl shadow-sm border border-zinc-100">
                <span className="text-3xl font-black text-zinc-900">{suggestedLabel || '—'}</span>
              </div>
            </div>

            {/* Tarjeta Seleccionado */}
            <div className={`flex flex-col items-center justify-center w-40 sm:w-48 p-4 rounded-3xl border-2 transition-all ${selectedPulse ? 'bg-brand-50 border-brand-400 shadow-[0_0_20px_rgba(45,212,191,0.4)]' : 'bg-zinc-50 border-zinc-200'}`}>
              <span className={`flex items-center gap-2 text-xs font-bold uppercase tracking-widest mb-3 ${selectedPulse ? 'text-brand-600' : 'text-zinc-400'}`}>
                <IconCheckCircle /> Seleccionado
              </span>
              <div className="bg-white px-6 py-2 rounded-xl shadow-sm border border-zinc-100">
                <span className="text-3xl font-black text-zinc-900">{slotSel?.label || '—'}</span>
              </div>
            </div>
          ...
          <button 
            ...
            className="w-full sm:w-auto px-12 py-4 bg-brand-500 hover:bg-brand-400 disabled:bg-zinc-200 disabled:text-zinc-400 text-white font-black text-lg rounded-2xl shadow-xl shadow-brand-500/30 transition-all flex items-center justify-center gap-2 active:scale-95"
          >
            {loading ? <div className="w-6 h-6 border-2 border-white/30 border-t-white rounded-full animate-spin"/> : (activeTab==='single' ? 'Guardar paquete' : `Guardar ${multiCount} paquetes`)}
          </button>
        </div>


        {/* ZONA UBICACIONES (Grid de Calor) */}
        <div>
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-4">
            <div>
              <label className="text-xs font-bold text-zinc-900 uppercase tracking-widest">Estructura del Local</label>
              <p className="text-xs text-zinc-500 font-medium mt-1">El sistema pre-selecciona el hueco óptimo.</p>
            </div>
            <div className="flex items-center gap-4 text-[10px] font-bold text-zinc-500 uppercase tracking-wider bg-zinc-50 px-3 py-1.5 rounded-lg border border-zinc-200">
              <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-zinc-300"/> Vacía</div>
              <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-emerald-400"/> Libre</div>
              <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-amber-400"/> Media</div>
              <div className="flex items-center gap-1.5"><div className="w-2 h-2 rounded-full bg-red-500"/> Llena</div>
            </div>
          </div>

          <div 
            className="grid gap-2 sm:gap-3 pb-8" 
          ...
          >
            {ubicaciones.map(u => {
              ...
              if (isSelected) {
                finalClasses = 'bg-zinc-950 border-zinc-950 text-white shadow-xl ring-4 ring-zinc-950/20 transform scale-110 z-20';
              } else if (isSuggested) {
              ...
              return (
                <button
                  ...
                  className={`
                    relative flex flex-col items-center justify-center py-3 sm:py-4 rounded-xl transition-all border-2 outline-none
                    ${finalClasses}
                  `}
                >
                  <span className="text-sm sm:text-lg font-black tracking-tight">{u.label}</span>
                  <span className={`text-[10px] sm:text-xs font-bold mt-0.5 ${isSelected ? 'text-zinc-400' : ''}`}>{count} paq.</span>
                </button>
              );
            })}
          </div>
      ...
      <AnimatePresence>
        {exito && (
          <motion.div 
            ...
            className="fixed bottom-8 left-1/2 bg-zinc-950 text-white px-6 py-4 rounded-2xl shadow-2xl font-bold flex items-center gap-3 z-[9999] border border-zinc-800 whitespace-nowrap"
          >
