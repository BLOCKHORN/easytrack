...
function StepOneIllustration() {
  return (
    <div className="relative w-full h-64 mb-10 flex items-center justify-center overflow-hidden">
      {/* Radial gradient en lugar de blur (100x más rápido) */}
      <div className="absolute w-64 h-64 bg-[radial-gradient(circle,rgba(99,102,241,0.15)_0%,transparent_70%)]" />
      <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} className="relative z-10 w-48 h-32 bg-zinc-900 border border-zinc-800 rounded-2xl p-4 shadow-2xl flex flex-col justify-between">
        <div className="flex items-center gap-2">
          <FaUserCircle className="text-brand-400 text-lg" />
          <div className="h-1.5 w-16 bg-zinc-700 rounded-full" />
        </div>
        <div className="space-y-2">
          <motion.div initial={{ width: 0 }} whileInView={{ width: '100%' }} transition={{ delay: 0.5, duration: 1.5 }} className="h-1 bg-brand-500 rounded-full" />
  ...
  return (
    <div className="relative w-full h-64 mb-10 flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 grid grid-cols-3 gap-3">
        {[...Array(6)].map((_, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} className={`w-12 h-16 rounded-xl border border-zinc-800 flex items-end p-2 ${i === 3 ? 'bg-brand-500/20 border-brand-500/30' : 'bg-zinc-900'}`}>
  ...
  return (
    <div className="relative w-full h-64 mb-10 flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center">
        <motion.div animate={{ y: [0, -8, 0] }} transition={{ duration: 3, repeat: Infinity }} className="px-6 py-4 bg-zinc-900 border border-zinc-800 rounded-2xl shadow-2xl">
          <FaBarcode className="text-5xl text-zinc-600 mb-2" />
          <motion.div animate={{ opacity: [0.3, 1, 0.3] }} transition={{ duration: 2, repeat: Infinity }} className="h-0.5 w-full bg-brand-500 shadow-[0_0_10px_#4f46e5]" />
        </motion.div>
      </div>
    </div>
  );
...
export default function HowItWorks() {
  ...
  return (
    <section id="como-funciona" className="relative bg-zinc-950 text-white pb-12 pt-32 overflow-hidden">
      ...
      {/* Ola Superior (Transición perfecta desde slate-50) */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-20 md:mb-28">
          <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-4xl md:text-6xl font-black tracking-tighter mb-6 text-white">
