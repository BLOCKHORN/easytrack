...
function Switch({ checked, onChange }) {
  return (
    <button
      ...
      className={`relative inline-flex h-6 w-11 flex-shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out focus:outline-none ${checked ? 'bg-brand-500' : 'bg-zinc-200'}`}
    >
      ...
      <header className="p-6 md:p-8 border-b border-zinc-100 bg-zinc-50/30 flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-zinc-500 font-medium text-sm mt-1 max-w-md">Define con qué compañías trabajas y el ingreso por entrega.</p>
          </div>
        </div>
      ...
      <div className="p-6 md:p-8 bg-zinc-50/50">
        {empresas.length === 0 && (
          <div className="text-center py-12 px-6 bg-white border-2 border-zinc-200 border-dashed rounded-2xl">
            ...
            <button className="px-6 py-3 bg-brand-500 hover:bg-brand-400 text-white font-bold text-sm rounded-xl transition-all shadow-lg flex items-center justify-center gap-2 mx-auto active:scale-95" onClick={doAdd}>
        ...
        <div className="space-y-4">
          {empresas.map((e, i) => {
            ...
            return (
              <article key={i} className="flex flex-col lg:flex-row lg:items-center gap-4 p-5 bg-white border border-zinc-200 rounded-2xl shadow-sm hover:border-zinc-300 transition-colors">
                ...
                <div className="flex flex-wrap sm:flex-nowrap items-center gap-4 w-full lg:w-auto">
                  <div className="flex-1 sm:w-40 sm:flex-none">
                    <label className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mb-1.5 flex items-center justify-between">
                      Ingreso <span className="text-zinc-300 normal-case tracking-normal">(Sin IVA)</span>
                    </label>
                    <select
                      value={e.ingreso_por_entrega ?? 0}
                  ...
                  <div className="flex flex-col items-center sm:ml-2">
                    ...
                    <button type="button" onClick={() => doRemove(i)} className="w-11 h-11 flex items-center justify-center bg-zinc-50 border border-zinc-200 text-zinc-400 hover:text-white hover:bg-red-500 hover:border-red-600 rounded-xl transition-all shadow-sm">
                      <IconTrash />
                    </button>
                  </div>
                </div>
