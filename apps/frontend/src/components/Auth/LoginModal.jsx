  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <motion.div 
        ...
        className="relative w-full max-w-[420px] rounded-3xl shadow-2xl overflow-hidden border will-change-transform transition-colors duration-500 bg-white border-zinc-100"
      >
        <div className="p-8 md:p-10">
          <div className="mb-8">
            <h2 className="text-2xl font-black tracking-tight mb-1 text-zinc-950">
              Inicia sesión
            </h2>
          </div>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <AnimatePresence mode="wait">
              {error && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="p-4 text-sm font-bold rounded-xl border bg-amber-50 text-amber-800 border-amber-200">
                  {error}
                </motion.div>
              )}
            </AnimatePresence>

            <button disabled={loading} className="w-full py-3.5 mt-2 text-white font-black text-lg rounded-xl transition-all shadow-xl active:scale-95 disabled:opacity-50 disabled:shadow-none disabled:active:scale-100 bg-zinc-950 hover:bg-zinc-800 shadow-zinc-950/10">
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-xs font-black uppercase tracking-widest text-zinc-400">
        ...
        <div className="text-center p-6 border-t transition-colors bg-zinc-50 border-zinc-100">
          <p className="text-sm font-medium text-zinc-500">
            ¿Eres nuevo en EasyTrack? <button onClick={() => { onClose(); navigate('/registro'); }} className="font-black transition-colors text-zinc-950 hover:text-brand-600">Crea una cuenta</button>
          </p>
        </div>
      </motion.div>
    </div>
